...
# week6
## 동시성 제어
- 여러 트랜잭션이 동시에 같은 데이터에 접근 시 발생 가능한 문제(쓰기-읽기, 쓰기-쓰기)를 방지
- 데이터 일관성

## 락
- 충돌을 미리 예상하고 데이터에 락을 걸어 다른 트랜잭션이 접근 못 하게 하는 방식
- 데이터 무결성
...
## 엔티티
- 테이블 구조 정의
- @Table: 엔티티를 어떤 데이터베이스 테이블과 매핑할지 설정하는 어노테이션
  - Order 클래스에서는 order가 DB 예약어이기에 해당 어노테이션을 사용하여 이름 지정
  - 사용하지 않으면 엔티티는 기본적으로 클래스명과 같은 이름의 테이블과 매핑
- @ManyToOne: 여러 개의 엔티티가 하나의 엔티티를 참조하는 관계를 나타낼 때 사용하는 어노테이션(부모 엔티티가 N, 자식이 1)
  - fetch
    - FetchType.LAZY: 연관된 엔티티를 바로 가져오지 않고, 실제로 접근할 때 쿼리를 날림
    - FetchType.EAGER: 연관된 엔티티도 바로 함께 조회
- @JoinColumn: 연관관계의 외래 키 컬럼을 명시적으로 지정하는 어노테이션
- @OneToMany: 하나의 엔티티가 여러 개의 엔티티를 참조하는 관계를 나타낼 때 사용하는 어노테이션(부모 엔티티가 1, 자식이 N)
  - mappedBy:양방향 매핑에서 연관관계의 주인이 아닌 쪽에 사용
    - @OneToMany 단독 사용 시 중간 테이블이 생성될 수 있기에 사용
  - cascade: 부모 엔티티의 변경을 자식에게 연쇄 전파 옵션
- 양방향 매핑
  - 양쪽 엔티티가 서로 참조하는 관계

## 레포지토리
- 데이터 조작
- @Query: JPA 기본 메소드로는 어려운 복잡한 쿼리를 직접 SQL로 작성할 수 있게 함
  - @Param(변수명)을 메소드의 파라미터로 주고 SQL문에 ```:변수명```으로 이용

## N+1 문제
- 연관관계로 인해 발생하는 쿼리 폭발 문제
- Fetch Join
  - 한방 쿼리, 가져올 때 다 조인해서 한 번에 가져옴

# week9
## QueryDSL
- SQL을 문자열이 아닌 자바 코드로 작성할 수 있게 해주는 쿼리 라이브러리
- Q파일 생성
  1. 의존성 추가
  2. Q-Type 클래스 생성
     - clean: 기존 빌드 파일 삭제
...
- 설정 파일
  - @Configuration: 설정 클래스 선언
    - @Bean 메소드를 찾아 빈 등록
  - @PersistenceContext: JPA의 EntityManager를 자동으로 주입
  - @Bean: 메소드가 반환하는 객체를 Spring 빈으로 등록
- 사용자 정의 레포지토리
  - 인터페이스: 기능 선언
  - 구현체: 기능 구현, 자바 코드로 쿼리 작성
    - JPAQueryFactory
    - .selectFrom: 어떤 테이블에서 데이터를 조회할지 지정
    - .join: 연관관계 기반으로 테이블 조인
    - .where: 조건 설정
    - .fetch: 실제 쿼리 실행 및 결과 반환
  - 메인 레포지토리: 인터페이스와 구현체를 상속받음

## DTO
- 계층 간 데이터를 전달하기 위해 사용
- record: 불변 데이터 객체를 간결하게 만드는 특별한 클래스

# week10
## 취소 로직
- DB에서 데이터를 지우는 것이 아닌 상태를 바꾸고, 다시 원상복구 시키는 것

## Enum
- 제한된 고정 값들의 집합을 표현하는 특별한 자바 클래스
- @Enumerated
  - enum 타입을 데이터베이스 컬럼에 매핑하는 방법을 지정하는 어노테이션
  - EnumType.String: enum 이름을 문자열로 저장

## AOP
- Aspect Oriented Programming
- 공통 관심사(로그, 트랜잭션, 보안 등)를 핵심 비즈니스 로직에서 분리하여 모듈화하는 프로그래밍 기법
- @Aspect: AOP 클래스 선언
- @Component: Spring 빈 등록
- @Around: 메소드 호출 전후에 공통 로직 실행
  - execution(* ...): 대상 지정